...
#include <windows.h>
int cnt = 0;
void InsertSort(int n, WIN32_FIND_DATA mass[]) //сортировка вставками
{
...
void MergeSort(int i, int j, WIN32_FIND_DATA a[], WIN32_FIND_DATA aux[]) //сортировка слиянием
{
    if (j <= i) {
        return;
    }
...
int main()
{
    ...
    TCHAR path[100]; // = { "D:\RL" }; путь к файлам
    ...
    DWORD dwError = 0;
    StringCchCopy(szDir, MAX_PATH, path); // strcpy
    StringCchCat(szDir, MAX_PATH, TEXT("\\*")); // strcat
    ...
    do {
        ...
        case 1:
            ...
            do {
                ...
                switch (ch1) {  //выбор метода сортировки(по возрастанию или убыванию)
                case 1:
                    for (int i = 0; i < cnt; ++i)
                    {
